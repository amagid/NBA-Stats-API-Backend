...
module.exports = function(req, res) {
    //If we're not supposed to save this search, jump out now
    if (!req.saveSearch) {
        return;
    }
...
function players(userId, originalUrlParts, params) {
    ...
    //Do the upsert
    return PlayerQueries.upsertSearch(userId, originalUrlParts.join('/'), command, params)
        .catch(err => {
...
function teams(userId, originalUrlParts, params) {
    ...
    //Do the upsert
    return TeamQueries.upsertSearch(userId, originalUrlParts.join('/'), command, params)
        .catch(err => {
            logger.error(JSON.stringify(err));
        });
}
...
function games(userId, originalUrlParts, params) {
    ...
    //Do the upsert
    return GameQueries.upsertSearch(userId, originalUrlParts.join('/'), command, params)
        .catch(err => {
            logger.error(JSON.stringify(err));
        });
}